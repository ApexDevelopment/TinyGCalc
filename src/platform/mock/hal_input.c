                ...
                switch (sym) {
                    ...
                    default: {
                        ...
                        if (sym >= SDLK_a && sym <= SDLK_z) {
                            ...
                            switch (sym) {
                                case SDLK_MINUS:    ch = shift ? '_' : '-'; break;
                                case SDLK_EQUALS:   ch = shift ? '+' : '='; break;
                                case SDLK_LEFTBRACKET:  ch = shift ? '{' : '['; break;
                                case SDLK_RIGHTBRACKET: ch = shift ? '}' : ']'; break;
                                case SDLK_BACKSLASH:    ch = shift ? '|' : '\\'; break;
                                case SDLK_SEMICOLON:    ch = shift ? ':' : ';'; break;
                                case SDLK_QUOTE:        ch = shift ? '"' : '\''; break;
                                case SDLK_COMMA:        ch = shift ? '<' : ','; break;
                                case SDLK_PERIOD:       ch = shift ? '>' : '.'; break;
